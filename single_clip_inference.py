...
parser.add_argument(
    "--submissions_folder",
    help="folder to save submissions",
    default=None,
    required=True)
args = parser.parse_args()

# Load the model
if args.mtype == "metaclip":
    print("loading meta clip")
    ...
    if args.clip_finetuned_model_name is not None:
        print("loading prev checkpoint")
        model = torch.load(os.path.join("checkpoints", args.clip_finetuned_model_name),map_location=lambda storage, loc: storage).to(device)
elif args.mtype == "dcxl":
    # if args.model_size == "large":
    print("training dcxl")
...
if args.phase == "trial":
    ...
    images_path = os.path.join("semeval-2023-task-1-V-WSD-train-v1", "train_v1", "train_images_v1")
elif args.phase == "val":
    input_folder_path = os.path.join("semeval-2023-task-1-V-WSD-train-v1", "val_v1")
...
if args.phase == "trial" or args.phase == "val":
    ...
    if not os.path.exists(args.submissions_folder):
        os.makedirs(args.submissions_folder)
    ...
    for version in versions:
        with open(os.path.join(args.submissions_folder, args.phase+"_submission_"+version+".txt"), "w") as f:
            pass
    ...
    with torch.no_grad():
        for index, row in tqdm(df.iterrows(), total=df.shape[0]):
            try:
                ...
                # if not isinstance(ambiguous,float):
                ambiguous_index = sentence.find(ambiguous)
                    # if not isinstance(ambiguous_index,float):
                ...
                for j, text_logits in enumerate(logits):
                    ...
                    with open(os.path.join(args.submissions_folder, args.phase+"_submission_"+versions[j]+".txt"), "a") as f:
